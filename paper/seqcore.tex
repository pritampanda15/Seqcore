...
$\PrimaryFasterHi\times$ in absolute time. \fi We additionally measure what a GPU
...
and \OtherMachine{} with \OtherCores{} vCPUs. Both run CPython \BenchPython,
NumPy \BenchNumpy{} and Biopython \BenchBiopython{} so that hardware is the only
difference between them; unless stated otherwise, reported numbers are from the
\PrimaryMachine, and Section~\ref{sec:res:platform} compares the two. We include
a desktop-class Apple silicon machine because it is increasingly the hardware on
