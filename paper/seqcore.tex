...
Unless stated otherwise, reported numbers are from the \PrimaryMachine. It is
not the faster of the two; it is the reproducible one. A dedicated instance is
...
part outperforming a cloud vCPU allocation is worth knowing when choosing where
...
Two of these results mean something different from the others. The molecule
functions wrap
...
Figure~\ref{fig:modules}b makes a different point. For tree building, the
user-facing choice of distance metric spans three orders of magnitude,
far more than any implementation detail in this paper. \texttt{identity} aligns
...
programming are all standard. They only become available once a batch of
...
those is a representation problem. What the two halves share is narrower and
perhaps more useful: in each case the measured time was going somewhere other
than the arithmetic the user asked for, and the fix was to find where.
