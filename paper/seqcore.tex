...
part outperforming a cloud vCPU allocation, which is worth knowing if you are
choosing where to run this kind of analysis --- but the sign of every comparison
is identical: the same three operations win, the same two lose, and the $k$-mer
...
The lesson is that a single-machine speedup figure should be read as one point
estimate, not a property of the library --- which is why we report two machines
and commit the harness that produced both.
\fi
...
Two results are worth separating from the rest. The molecule functions wrap
...
conversion at a language boundary, and the absence of a spatial index --- none
of which is a representation problem. What the two halves share is narrower and
