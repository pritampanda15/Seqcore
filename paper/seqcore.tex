...
$\AlignSlower\times$ slower than Biopython's compiled aligner. \ifMultiMachine Results are
reported on \NumMachines{} machines, a cloud Linux instance and a desktop
Apple silicon Mac, which agree on every conclusion while differing by up to
$\OtherFasterHi\times$ in absolute time. \fi We additionally measure what a GPU
...
\noindent
\textbf{Scope.} Beyond sequence operations we apply the same analysis to
Seqcore's molecule, structure and phylogenetics modules, where the limiting
costs turn out to be repeated parsing and missing spatial indices rather than
data layout, and where the appropriate target for a wrapper around an
established library is parity rather than advantage.
...
function-call overhead) rather than by the residue arithmetic the user actually
asked for. The effect is invisible at the scale of a handful of
...
the interpreter cost is $O(mn)$, which is the access pattern NumPy handles
least efficiently, applied to the largest number of operations in the library.
...
Two properties of this layout matter for what follows. First, any
...
answered once by the boolean mask
...
\ifMultiMachine
Measurements were taken on two machines: \BenchHost{} running \BenchPlatform,
and \OtherMachine{} with \OtherCores{} cores. Both run CPython \BenchPython,
NumPy \BenchNumpy{} and Biopython \BenchBiopython, so hardware is the only
difference between them, and Section~\ref{sec:res:platform} compares the two. We
include a desktop-class Apple silicon machine because it is increasingly the
hardware on which this kind of analysis is actually run.

Unless stated otherwise, reported numbers are from the \PrimaryMachine, which is
the slower of the two. We chose it for reproducibility. A dedicated instance is
otherwise idle, whereas a desktop is generally also being used for something
else, and that shows up in the measurements: on the dedicated instance the
Seqcore-to-Biopython ratios agree to within 2\% across runs made fourteen hours
apart, while on the desktop the same ratios moved by 35--52\% between an idle
session and a busy one. Absolute times are lower on the desktop and are reported
in Section~\ref{sec:res:platform}; we would not want them quoted as headline
figures, because we cannot promise a reader would reproduce them.
\else
Measurements were taken on \BenchHost{} running \BenchPlatform, with CPython
\BenchPython, NumPy \BenchNumpy{} and Biopython \BenchBiopython. We report a
desktop-class Apple silicon machine because it is increasingly the hardware on
which this kind of analysis is actually run.
\fi
Each configuration runs in a
...
repetitions (three for alignment and $k$-mer counting), which is the
appropriate statistic for throughput on a machine that is not otherwise
quiesced. Sequences
...
whole range. It is flat, as expected for compiled code with no per-call
interpreter cost. Seqcore does not approach this and is not intended to;
...
for each operation\ifMultiMachine{} on each machine measured\fi, at the
configurations in Table~\ref{tab:summary}. Bars left of the dashed line are
operations where Seqcore is slower.\ifMultiMachine{} The ordering of operations
is the same on both machines.\fi}
\label{fig:kmers}
\end{figure}
...
Absolute times depend strongly on the machine. Whether any conclusion does is a
separate question, and Figure~\ref{fig:kmers}b repeats the comparison on both
machines. Seqcore runs $\OtherFasterLo$--$\OtherFasterHi\times$ faster on the
\FasterMachine{} than on the dedicated instance in absolute terms. A desktop
part outperforms a cloud vCPU allocation here, which matters when choosing where
to run this kind of analysis. The sign of every comparison is nonetheless
identical: the same three operations win, the same two lose, and the $k$-mer
dispatch crossover falls at the same $k$.

The magnitudes do move, and in both directions. GC content is
$\GCvsBio\times$ Biopython on the dedicated instance against
$\OtherGCvsBio\times$ on the desktop, while translation goes the other way,
$\TransVsBio\times$ against $\OtherTransVsBio\times$: the two machines do not
merely scale by a constant, because the operations stress memory bandwidth and
interpreter dispatch in different proportions. Pairwise alignment is
$\AlignSlower\times$ slower than Biopython on one and $\OtherAlignSlower\times$
on the other.

A single-machine speedup figure is therefore one point estimate rather than a
property of the library. We report two machines and commit the harness that
produced both.
\fi

\subsection{Beyond sequence operations}
\label{sec:res:modules}

Seqcore also provides modules for small molecules, structural biology,
phylogenetics and population genetics. These do not share the sequence array
representation, so they are a useful test of how far the paper's argument
carries. Figure~\ref{fig:modules} compares them against the library a user would
otherwise reach for.

The bottlenecks there turned out not to be representation at all. Molecular
property functions re-parsed the SMILES string on every call, so the cost was
repeated work rather than the wrong data structure; caching the parsed molecule
removed roughly a factor of forty. Morgan fingerprints spent almost all their
time converting RDKit bit vectors to arrays one bit at a time in Python. Solvent
accessible surface area tested every atom against every other for every surface
point, and re-derived van der Waals radii from element strings inside that
innermost loop; restricting each atom to the neighbours whose inflated sphere
can reach it makes the cost roughly linear rather than quadratic, and at 5000
atoms takes it from tens of minutes to a fraction of a second.

The molecule functions wrap
RDKit, and the structural neighbour searches wrap a spatial index; for those the
honest target is parity, not victory, and Figure~\ref{fig:modules}a shows them
sitting below $1$. A wrapper that costs a few tens of percent buys API
consistency at a defensible price; the same wrapper costing seventy times was
simply a defect.

For tree building, the user-facing choice of distance metric spans three orders
of magnitude (Figure~\ref{fig:modules}b), far more than any implementation
detail in this paper. \texttt{identity} aligns
every pair with Needleman--Wunsch, which is correct for unaligned input and
costs $O(n^2L^2)$; \texttt{hamming} compares an existing alignment column by
column. The two are not interchangeable, and disagree once sequences diverge
enough for the aligner to open gaps, so the slower and more general option
remains the default. We note it because a benchmark that pairs one library's
\texttt{identity} against another's without checking that they denote the same
computation will report a difference that is an artefact of the comparison
rather than of either implementation.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig6_modules.pdf}
\caption{\textbf{Domain modules against their reference libraries.}
\textbf{a}, Seqcore relative to the library a user would otherwise use for the
same operation, at the largest size measured; the reference is named beside each
bar. Bars left of the dashed line are operations where the reference is faster.
The molecule functions wrap RDKit and the structural searches wrap a spatial
index, so parity rather than advantage is the target there.
\textbf{b}, Neighbour-joining against Biopython. The choice of distance metric
dominates: \texttt{identity} aligns every pair before building the tree, while
\texttt{hamming} compares an already-aligned input column by column. Biopython's
identity calculator does the latter, so only the lower two curves are a
...
Two operations do not favour this design.

\textbf{Reverse complement} runs at $\RCvsBio\times$ Biopython ($\RCms$\,ms versus
$\RCbioms$\,ms at $\BatchN$ sequences\ifMultiMachine; $\OtherRCvsBio\times$ on
the \OtherCPU\fi). Biopython implements it as \texttt{str.translate}
...
aligner at $L=\AlignLenHi$\ifMultiMachine{} (and $\OtherAlignSlower\times$
slower on the \OtherCPU)\fi. The wavefront still issues $O(m+n)$ NumPy calls, each materializing
...
Within the domain modules, \texttt{nucleotide\_diversity} and
\texttt{tajimas\_d} remain quadratic in the number of sequences, and tree
construction itself is still an interpreted $O(n^3)$ loop, which becomes the
limiting cost once the distance matrix is no longer the bottleneck.

Seqcore's \texttt{align()} also accepts a
...
kernels to CuPy~\cite{okuta2017cupy} is mechanically straightforward, so
measuring what it would buy is cheaper than undertaking it. We
...
Measured with the matrix already resident on the device, the kernels are 23--326$\times$ faster than
Seqcore's CPU path. Measured per call from host memory, which is what a user
calling \texttt{sc.reverse\_complement(arr)} would actually experience, the same
kernels are 5--20$\times$ faster, because transferring the batch across
...
chained operations already double the benefit. $k$-mer counting begins near its
ceiling.

A GPU backend implemented per function, with each call copying in and out, would deliver roughly $5\times$ on the
...
limits this class of Python library. None of the techniques used here are
novel: masked reductions, lookup tables and wavefront dynamic programming are
all standard. They only become available once a batch of sequences is a dense
numeric array rather than a list of objects. Choosing that
representation first is what makes the rest follow.

The domain modules in Section~\ref{sec:res:modules} qualify that claim rather
than confirming it. There the costs were repeated parsing, element-wise
conversion at a language boundary, and the absence of a spatial index. None of
those is a representation problem. The two halves share something narrower: in
each case the measured time was going somewhere other than the arithmetic the
user asked for, and the work was finding where.

Two points generalize beyond this library. An optimization needs a stated
domain of applicability. The $k$-mer result is the clearest case:
...
Reporting the unfavourable comparisons is part of the contribution. A library
that is $\GCvsBio\times$ faster
...
the favourable half would make the library harder to use correctly.
